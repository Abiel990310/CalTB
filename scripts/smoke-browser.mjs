...
await page.goto('http://localhost:4199/limits/computing-limits/', { waitUntil: 'networkidle' });
...
await page.locator('.part__input').first().fill('16/2');
await page.locator('.part__check').first().click();
console.log('answer "16/2" ->', await page.locator('.part__verdict').first().textContent());

// A wrong one.
await page.locator('.part__input').nth(1).fill('1/3');
await page.locator('.part__check').nth(1).click();
console.log('answer "1/3"  ->', await page.locator('.part__verdict').nth(1).textContent());

console.log('progress now:', await page.locator('.exercise__progress').textContent());

// Typing again must retract the stale verdict.
await page.locator('.part__input').first().fill('16/');
console.log('verdict hidden after edit:', await page.locator('.part__verdict').first().isHidden());

// The standalone page too.
await page.goto('http://localhost:4199/practice/antiderivatives/', { waitUntil: 'networkidle' });
await page.waitForSelector('.part__input', { timeout: 5000 });
await page.locator('.part__input').nth(3).fill('-cos(2x)/4');
await page.locator('.part__check').nth(3).click();
console.log('sin·cos part, answer "-cos(2x)/4" ->', await page.locator('.part__verdict').nth(3).textContent());
